...
case "$REL" in wrangler.jsonc|worker-configuration.d.ts|workers/*|workflows/*) add cloudflare ;; esac
...
why_for() {
  case "$1" in
    frontend)   echo "UI, forms (Effect Schema + effectResolver — no Zod), modals, Tailwind / CSS variables" ;;
