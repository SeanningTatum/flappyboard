...
- run note: none
- next: app.css token pass with the max(0px, calc()) radius guard first (or .rounded-sm silently dies app-wide), then [data-surface=hardware] on the four console routes, self-hosted fonts with static TV cuts, --font-flap pin behind FLAP_GLYPH_CLASS, focus-ring fix + delete the two hardcoded workarounds at link.tsx:346-354 and sound-unlock-prompt.tsx:48.

---

## 2026-07-31 — Brand/IA/UX redesign planned and approved in review round 1 (plans/2026-07-31-brand-ia-redesign.html, session ended by owner). Research found the load-bearing problem: TWO design systems, only one good — app/app.css is stock shadcn neutral unmodified (achromatic, radius 0.625rem, Inter from Google CDN at root.tsx:21-32) while flap-tile/board-frame/console carry a researched hardware language (tonal ladder, hairlines, 1px lips, 8 pigments measured off a real Vestaboard with PIL) entirely in hardcoded hex, outside the token contract, never using the documented [data-surface] escape hatch. Board render FROZEN as the reference, not the subject. 15 decisions locked: no /dashboard, hybrid tokens (global contract + [data-surface=hardware]), radius 2px, Archivo + IBM Plex Mono self-hosted, collapse the /link receipt, inline device naming, landing board flips once, add a Boards link to the admin sidebar, KEEP file-upload component+route (owner reversed my delete recommendation), one board per TV, thin /boards rack as a switcher, name stays flappyboard, no first-pair greeting, device count on the controller, REFERO_MCP_TOKEN not pursued (research runs degraded on bundled craft refs + three existing dated locks). Round 1 cut MORE than proposed: the board-count branch dies entirely — resolveAutoLink, the picker, the select and /link's manual mode all delete; a board IS a TV; scan lands you on the controller with nothing to answer (5 stops -> 3). Board detail folds into the controller as Content|Settings tabs at the frozen /b/:id/c path. Standing constraint from the approval: the phone is the controller, so thumb reach and target size outrank visual preference. THREE defects found that were not part of the ask: (1) non-admin users CANNOT SIGN OUT — authClient.signOut() exists only at admin/layout/nav-user.tsx:49; (2) the /:lng routes are a soft-404 farm — no pathname detector in i18nServer, no route reads params.lng, and :lng is a catch-all so /pricing returns 200 with the marketing page; (3) the recorded focus-ring diagnosis is WRONG — --tw-ring-shadow does reach box-shadow (verified in compiled CSS); the real defect is --ring oklch(0.708 0 0) at 50% alpha composing to ~1.9:1 on white against a 3:1 floor, so the ring paints and is invisible.
- branch: `main`
- in-progress feature: none
- run note: none
- next: Phase 1 brand-system: branch from a clean tree (wrangler.jsonc stays uncommitted and out of all PRs), brain features set-status brand-system --status in-progress, then app.css token pass (radius 2px WITH the max(0px,calc()) guard or .rounded-sm silently dies app-wide), [data-surface=hardware] applied on the four console routes, Archivo + IBM Plex Mono self-hosted from app/assets/fonts with static cuts on the TV path (Tizen is Chromium 56, variable fonts need 62+), --font-flap pinned to Inter behind a shared FLAP_GLYPH_CLASS so --font-sans cannot leak into the frozen board, focus-ring fix + delete the two hardcoded workarounds at link.tsx:346-354 and sound-unlock-prompt.tsx:48, console.tsx re-pointed at var(--hw-*) with NO export-shape change, TILE_COLORS<->--flap-* parity test, design-system.md rewritten. Gate each phase with design:audit + feature-verifier PASS + design-critic (memorability and identity verdicts asked by name; P0/P1 blocking). Extend BOARDS_KEYS in board-locale-parity.test.ts BEFORE moving any copy — its coverage is 7 keys, only the revoke dialog. Watch: jsdom does not resolve custom properties, so never assert on computed colors. Open for phase 3: board sprawl when a cookie-evicted TV is re-scanned — one secondary 'attach to an existing board' line, NOT a rebuilt picker.
